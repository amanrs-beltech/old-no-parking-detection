...
def draw_track_bbox(image, bboxes, CLASSES=read_class_names(cfg.YOLO.CLASSES), show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    NUM_CLASS = CLASSES
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            label = "{}".format(NUM_CLASS[class_ind]) +" "+direction
...
def bbox_details(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        # print(out_classes[0][i])
        current_bbox.extend([coor[1],coor[0],coor[3],coor[2],out_scores[0][i],class_ind])
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        current_bbox["right"] = coor[3] 
        current_bbox["bottom"] = coor[2] 
        current_bbox["top"] = coor[0] 
        current_bbox["score"] = out_scores[0][i]
        class_ind = int(out_classes[0][i])
        current_bbox["id"] = classes[class_ind]
        current_bbox["left"] = coor[1] 
        detections.append(current_bbox)
